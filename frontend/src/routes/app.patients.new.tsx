...
function NewPatient() {
  ...
  // Block patient from creating a second profile
  const { data: existingPatients } = useQuery<Patient[]>({
    queryKey: ["patients"],
  ...
  useEffect(() => {
    if (isPatient && existingPatients && existingPatients.length > 0) {
      navigate({ to: "/app/patients/$id", params: { id: existingPatients[0].id }, replace: true });
    }
  }, [isPatient, existingPatients, navigate]);
